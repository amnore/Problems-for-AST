...
int main()
{
    ...
    int arr[3][10] = {{5, 10, 20, 80, 320, 640, 1000, 1000, 1000, 1000},
                      ...
                      {2200, 2200, 2400, 2600, 2800, 3000, 3000, 3000, 3000, 3000}};
    for (int i = 0; i < 3; ++i)
    {
        char sym;
        switch (i)
        ...
        for (int j = 0; j < 10; ++j)
        {
            ...
            {
                int n = std::uniform_int_distribution<>{1, std::min(arr[i][j] - current, std::max(arr[i][j] / 5, 100))}(re);
                current += n;
                ...
                {
                    tmp += delta;
                    fout << std::fixed << std::setprecision(std::numeric_limits<double>::digits10) << tmp << ' ' << a * tmp * tmp + b * tmp << '\n';
                }
            }
            ansout << max_n << '\n';
        }
